...
const resolvers = {
  Query: {
    ...
    completedReports: async (parent, args, context) => {
      if (context.user) {
        return Report.find({ inspectionDate: { $ne: null } }).populate('assignedBy').populate('assignedStaff').populate({ path: 'results', populate: { path: 'equipmentId' } }).populate({ path: 'roomId', populate: { path: 'location', populate: { path: 'client' } } }).sort({ inspectionDate: -1 });
      }
    },
